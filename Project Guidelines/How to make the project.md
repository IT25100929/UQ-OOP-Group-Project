...
they are using axios too in the frontend



@RequestPart - in POST to get the image to the backend



MultipartFile - image file type



how to POST the image and store in the database



\#26

how to GET the image stored in the database





### *whats the difference in return in POST and return in GET*



The fundamental difference isn't about *how* the data is sent back (both use the HTTP response body), but rather the **intent** and the **source** of that data.

### 1\. Return in a GET Method (The "Fetch")

A `GET` request is designed to be **idempotent**, meaning it should only retrieve data and never change the state of your database.

* **Source:** The data usually comes directly from your H2 database based on a search or a list request.
* **Intent:** "Show me what is already there."
* **Client Expectation:** The frontend expects a single object or a list of objects that represent the current state of the system.

> \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*Example:\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\* You ask for `GET /api/users/1`. The return is the user data for ID 1.

\---

### 2\. Return in a POST Method (The "Receipt")

A `POST` request is designed to **create** something new. It is "non-idempotent"—every time you call it, a new record is usually created.

* **Source:** The data is usually the result of the processing that just happened. It often includes data the client *didn't* have yet, like an auto-generated Primary Key (`id`), a creation timestamp, or a default status (e.g., `"status": "PENDING"`).
* **Intent:** "Here is a confirmation/receipt of exactly what I just saved."
* **Client Expectation:** The frontend expects the "Final Version" of the object it just sent.

> \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*Example:\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\* You send a name to `POST /api/users`. The return is the name \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*plus\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\* the `id: 502` generated by H2.

\---

### Comparison Table

|Feature|GET Return|POST Return|
|-|-|-|
|**Purpose**|To read existing data.|To confirm a creation/action.|
|**Data Origin**|Existing records in the database.|The newly created/processed record.|
|**Effect on Database**|None (Read-only).|State change (Data added).|
|**Typical HTTP Status**|`200 OK`|`201 Created`|

\---

### Why the difference matters for your React frontend

In your React app, the way you handle these returns will differ:

* **Handling GET:** You usually take the returned data and put it directly into a state variable to display it on a page (e.g., a list of products).
* **Handling POST:** You take the returned data to update your UI **without refreshing**. For example, if you just added a new "Task" to a Todo list, the `POST` returns that new Task with its database ID. You can then push that one object into your existing list so the user sees it immediately.



Think of it like a restaurant:

* **GET** is like asking for the menu. You get a list of what's available.
* **POST** is like placing an order. You get back a receipt confirming exactly what you ordered and your order number.

In your Spring Boot project, are you currently returning the whole entity object in your `POST` mapping, or just a boolean/string to say it worked?





i think getImageDate is a mistake in the video, but it works because in the database also instead of data they have used date.



\#27

5:11 watch from here



\#30

2:16 watch from here



\#31

When spring security is enabled, to post, put and delete we have to send CSRF token.



\#32

create a config class to handle security as we want

@configuration

create config classes in a new package

@EnableWebSecurity - this avoid default spring security so we can implement our own

@Bean - to customize security filter chain



\#33

packages - config, controller, service, repository, model



\#35

we use JWT so we dont have to provide credentials when going to every page of the website #37 11:30

13:26 - By JWT, we are not achieving secrecy of data, but knowing who send what (Asymmetric Key encrycption)



\#36

watched this video but didn't understand properly. Need to watch again.



\#37

to generate the token for every user, we can use their details like username, creationdate, expirdate etc and also the signature
...
install nodejs - it execute javascript code outside of a web browser. we are interested in the npm(node package manager) that comes with it
...
unpacking values from an object into variables



normally we write as,
...
but we can write that in one line as,
...
**Spread Operator,** - creating new object as an extend of other objects, like creating new arrays also by including the elements in another array
...
map() can be used to turn data into UI



we can look at every object and return a UI for each object

this is used all the time in react



find() - can pick specific item

includes() - can check conditions



**Ternary Operator** - if statement in one line of code



**Logical \&\& Operator -** if its only true then do this



**Optional chaining and nullish coalescing**

Optional chaining (one question mark) - access a value only if it exists

nullish coalescing (two question marks)- when the value if null or undefined



**Modules**

5:18 - 5.55 important topic
